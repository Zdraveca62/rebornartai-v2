  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
            ...
            topYouTube.map((item, idx) => (
              <a key={item.youtube_id || idx} href={item.videoUrl || `https://youtube.com/watch?v=${item.youtube_id}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', padding: '0.5rem 0', borderBottom: '1px solid rgba(168,85,247,0.2)' }}>
                  ...
                  <img src={getThumbnail(item)} alt={item.title} style={{ width: '45px', height: '45px', objectFit: 'cover', borderRadius: '6px' }} onError={(e) => { const yid = item.youtube_id; e.target.src = `https://img.youtube.com/vi/${yid}/mqdefault.jpg`; }} />
                  <div style={{ flex: 1 }}><div style={{ color: 'white', fontSize: '0.9rem', textTransform: 'uppercase' }}>{item.title?.substring(0,28)}</div><div style={{ color: '#aca1b7', fontSize: '0.75rem', textTransform: 'uppercase' }}>{item.views?.toLocaleString()} гледания</div></div>
                </div>
              </a>
            ))
          )}
        ...
        {/* TOP 5 SITE */}
        <div style={{ 
          background: 'radial-gradient(circle at 80% 30%, #3b0764, #52035a)', 
          borderRadius: '20px', padding: '1rem 1.2rem', 
          border: '1px solid rgba(243, 114, 211, 0.68)',
          marginBottom: '2rem' }}>
          ...
          <p style={{ color: '#c084fc', textAlign: 'right', marginBottom: '1rem', fontSize: '0.9rem', textTransform: 'uppercase' }}>{type === 'videos' ? 'MOST VIEWED' : 'MOST PLAYED'}</p>
          {topSite.length === 0 ? (
            ...
            topSite.map((item, idx) => (
              <a key={item.id || idx} href={`https://youtube.com/watch?v=${item.youtube_id}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', padding: '0.5rem 0', borderBottom: '1px solid rgba(168,85,247,0.2)' }}>
                  <img src={getThumbnail(item)} alt={item.song_title} style={{ width: '45px', height: '45px', objectFit: 'cover', borderRadius: '6px' }} onError={(e) => { if(item.youtube_id) e.target.src = `https://img.youtube.com/vi/${item.youtube_id}/mqdefault.jpg`; else e.target.src = PLACEHOLDER_IMG; }} />
                  <div style={{ flex: 1, textAlign: 'right' }}><div style={{ color: 'white', fontSize: '1rem', textTransform: 'uppercase' }}>{item.song_title?.substring(0,24)}</div><div style={{ color: '#c2a3e1', fontSize: '0.8rem', textTransform: 'uppercase' }}>{type === 'videos' ? `🎬 ${item.listen_count?.toLocaleString()} гледания` : `${item.song_language === 'bg' ? '🇧🇬 БЪЛГ' : '🇬🇧 EN'} • 🎵 ${item.listen_count}`}</div></div>
