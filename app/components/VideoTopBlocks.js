    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem', marginBottom: '2rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
        ...
        <div style={{
          ...
          <p style={{ color: '#b77eec', textAlign: 'left', marginBottom: '1rem', fontSize: '0.7rem', textTransform: 'uppercase' }}>
            MOST VIEWED
          </p>
          {topYouTube.length === 0 ? (
              ...
              <a key={item.id} href={`https://youtube.com/watch?v=${item.youtube_id}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', padding: '0.5rem 0', borderBottom: '1px solid rgba(168,85,247,0.2)' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ color: '#a855f7', fontSize: '0.65rem', textTransform: 'uppercase' }}>▶ {item.views?.toLocaleString()} гледания</div>
                  </div>
                </div>
              </a>
            ))
        ...
        <div style={{
          ...
          <p style={{ color: '#a855f7', textAlign: 'right', marginBottom: '1rem', fontSize: '0.7rem', textTransform: 'uppercase' }}>
            MOST VIEWED IN SITE
          </p>
          {topSite.length === 0 ? (
              ...
              <a key={item.id} href={`https://youtube.com/watch?v=${item.youtube_id}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem', padding: '0.5rem 0', borderBottom: '1px solid rgba(168,85,247,0.2)' }}>
                  ...
                  <div style={{ flex: 1, textAlign: 'right' }}>
                    <div style={{ color: 'white', fontSize: '0.8rem', textTransform: 'uppercase' }}>{item.title?.substring(0, 24)}</div>
                    <div style={{ color: '#a855f7', fontSize: '0.65rem', textTransform: 'uppercase' }}>🎬 {(item.site_views || 0).toLocaleString()} гледания</div>
                  </div>
