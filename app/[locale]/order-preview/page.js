...
export default function OrderPreviewPage() {
  ...
  // ── Redirect ако няма достъп ──────────────────────────────────────────────
  useEffect(() => {
    if (isLoading) return;
    if (level < 2) {
      router.replace(`/${locale}/ai-videos/Clients`);
    }
  }, [isLoading, level]);

  // ── Проверка дали вече има preview поръчка ────────────────────────────────
  useEffect(() => {
  ...
  // ── Ако вече има платена preview поръчка ─────────────────────────────────
  if (existingOrder) {
    return (
      <main style={{
        minHeight: '100vh',
        ...
        <div style={{
          ...
          <h2 style={{ color: '#fff', marginBottom: '0.5rem', fontSize: '1.4rem' }}>
            {locale === 'bg'
              ? `Добре дошъл обратно, ${user?.full_name?.split(' ')[0] || ''}!`
              : `Welcome back, ${user?.full_name?.split(' ')[0] || ''}!`}
          </h2>

          <p style={{ color: '#9ca3af', fontSize: '0.9rem', marginBottom: '2rem', lineHeight: 1.6 }}>
            {locale === 'bg'
              ? 'Вече имаш завършено видео превю. Готов ли си да направиш своята пълна поръчка?'
              : 'You already have a completed video preview. Ready to place your full order?'}
          </p>
          ...
          <div style={{
            ...
            <p style={{ color: '#60a5fa', fontSize: '0.82rem', margin: 0 }}>
              📋 {locale === 'bg' ? 'Твоето превю' : 'Your preview'}
            </p>
            <p style={{ color: '#e5e7eb', fontSize: '0.9rem', marginTop: '0.25rem' }}>
              🎭 {existingOrder.scene_name || (locale === 'bg' ? 'Избран сюжет' : 'Selected scene')}
            </p>
            <p style={{ color: '#6b7280', fontSize: '0.78rem', marginTop: '0.25rem' }}>
              📅 {new Date(existingOrder.created_at).toLocaleDateString(
                locale === 'bg' ? 'bg-BG' : 'en-IE'
              )}
            </p>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <button
              onClick={() => router.push(`/${locale}/full-order`)}
              style={{
                width: '100%',
                padding: '0.85rem',
                borderRadius: 10,
  ...
  return (
    <main style={{
      ...
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
              ...
              <div style={{
                ...
                background: step === s.key
                  ...
                  : Object.values(STEPS).indexOf(step) > i
                    ? 'rgba(59,130,246,0.3)'
                    : '#1a1a1a',
                color: step === s.key ? '#fff' : '#6b7280',
                border: '1px solid',
                borderColor: step === s.key ? '#3b82f6' : '#2a2a2a',
                transition: 'all 0.3s',
              }}>
        ...
        {/* Стъпка 2 — Качване на снимки */}
        {step === STEPS.PHOTOS && (
          <PhotoUpload
            photos={photos}
            onPhotosChange={setPhotos}
            onBack={() => setStep(STEPS.SCENE)}
            onNext={() => setStep(STEPS.SUMMARY)}
            locale={locale}
          />
        )}

        {/* Стъпка 3 — Преглед и плащане */}
        {step === STEPS.SUMMARY && (
          <PreviewOrderSummary
            scene={selectedScene}
            photos={photos}
            user={user}
            onBack={() => setStep(STEPS.PHOTOS)}
