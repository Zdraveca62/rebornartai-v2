...
export default function VideoTopBlocks({ category }) {
  ...
  // Топ 5 YouTube + Топ 5 Сайт — двата от /api/youtube-top
  useEffect(() => {
    ...
    fetch(url)
      ...
      .then(data => {
        if (data.success && data.videos) {
          // YouTube топ — сортирано по views (API вече го прави)
          setTopYouTube(data.videos.slice(0, 5));
          // Site топ — същите данни, views идват от youtube_cache
          // (site_views вече не съществува като отделна колона)
          setTopSite([...data.videos].slice(0, 5));
        } else {
          setTopYouTube([]);
          setTopSite([]);
        }
      ...
      .catch(err => {
        console.error('Грешка VideoTopBlocks:', err);
        setTopYouTube([]);
        setTopSite([]);
      })
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1rem', marginBottom: '2rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
            ...
            topYouTube.map((item, idx) => (
              <a key={item.youtube_id || idx}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem',
                  ...
                  <img src={getThumbnail(item.youtube_id)} alt={item.title}
        ...
        {/* СИН ПАНЕЛ — Сайт */}
        <div style={{ background: 'radial-gradient(circle at 80% 30%, #3b0764, #1e1b4b)',
                      borderRadius: '20px', padding: '1rem 1.2rem',
                      border: '1px solid rgba(168,85,247,0.4)' }}>
          <h2 style={{ color: '#d8b4fe', textAlign: 'right', marginBottom: '0.2rem',
                       fontSize: '1.3rem', textTransform: 'uppercase' }}>TOP 5 VIDEOS</h2>
          <p style={{ color: '#a855f7', textAlign: 'right', marginBottom: '1rem',
                      fontSize: '0.9em', textTransform: 'uppercase' }}>MOST VIEWED IN SITE</p>
          {topSite.length === 0 ? (
              ...
              <a key={item.youtube_id || idx}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.7rem',
                  ...
                  <div style={{ flex: 1, textAlign: 'right' }}>
                    ...
                    <div style={{ color: '#a855f7', fontSize: '0.75rem', textTransform: 'uppercase' }}>🎬 {(item.views || 0).toLocaleString()} гледания</div>
                  </div>
