...
export default function ClientsPage() {
  ...
  async function handleSignup(e) {
    ...
    try {
      ...
      showMessage('✅ Регистрацията е успешна! Моля, влезте с вашата парола.', 'success')
      setMode('login')
  ...
  return (
    <main style={{
      ...
      <div style={{
            ...
            <div>
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input type="email" value={email}
                  ...
                  disabled={emailLocked} required placeholder="вашият@имейл.com"
                  style={{
                    width: '100%', padding: '0.65rem 0.9rem', borderRadius: 8,
                    border: '1px solid #333', background: emailLocked ? '#111' : '#222',
                    color: emailLocked ? '#6b7280' : '#fff', fontSize: '0.95rem',
                    boxSizing: 'border-box', outline: 'none',
                  }} />
                {emailLocked && (
                  <span style={{ color: '#6b7280', fontSize: '0.75rem' }}>🔒 Разпознат имейл от Вашия IP</span>
                )}
              </div>
              ...
              <button
                ...
                style={{
                  ...
                  background: ipData?.failed_attempts >= 3 && ipData?.is_public_ip ? '#374151' : '#3b82f6',
                  color: '#fff', fontWeight: 600, fontSize: '1rem',
                  cursor: ipData?.failed_attempts >= 3 && ipData?.is_public_ip ? 'not-allowed' : 'pointer',
        ...
        {loading && (
          <div style={{ textAlign: 'center', color: '#6b7280', padding: '2rem 0' }}>⏳ Проверка...</div>
        )}
      </div>
    </main>
  );
